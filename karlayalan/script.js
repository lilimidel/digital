...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getAkmFromUrl() {
    ...
    return params.get("akm")?.toLowerCase();
  }

  let akm = getAkmFromUrl();
  if (akm) localStorage.setItem("akm", akm);
  else akm = localStorage.getItem("akm");

  // --- Determinar máximo de invitados ---
  let maxInvitados = 7;
  if (akm && letrasAMax[akm]) maxInvitados = letrasAMax[akm];

  // --- Filtrar opciones del select ---
  Array.from(select.options).forEach(option => {
    const val = parseInt(option.value, 10);
    if (!isNaN(val) && val > maxInvitados) option.remove();
  });
